...
@section('content')
    ...
    <div class="bg-white rounded shadow p-4 mb-6">
        <div class="mb-4 pb-2 border-b flex items-center">
            ...
            <h2 class="text-lg font-semibold">Create Collection Schedule</h2>
        </div>
            ...
            <form action="{{ route('admin.schedules.store') }}" method="POST">
                    ...
                    <div>
                        ...
                        <select name="waste_type" id="waste_type" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            ...
                            @foreach($wasteTypes as $value => $label)
                                <option value="{{ $value }}" {{ old('waste_type') == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="date" id="date" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{ old('date') }}" required>
                        @error('date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="notes" id="notes" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" rows="3">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-2 mt-6">
                    ...
                    <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded text-sm">Create Schedule</button>
                </div>
            </form>
        </div>
    </div>

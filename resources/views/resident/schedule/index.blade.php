    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-gray-600 mt-1 text-sm">Check waste collection schedules for your area</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6">
      ...
      <form method="GET" action="{{ route('resident.schedule.index') }}" id="scheduleFilters">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <select name="area_id" 
                      ...
                      class="w-full pl-10 pr-8 py-2.5 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 transition-all duration-200 bg-white text-sm appearance-none">
        ...
        @if($area)
        <div class="mt-4 flex items-center justify-between p-3 bg-gradient-to-r from-emerald-50 to-green-50 rounded-lg border border-emerald-200">
          <div class="flex items-center space-x-2">
            <svg class="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
            </svg>
            <span class="text-sm font-medium text-emerald-700">Selected Area:</span>
            <span class="text-sm font-bold text-emerald-800">{{ $area->name }}</span>
          </div>
        </div>
        @endif
      </form>
    ...
    @if(!$selectedAreaId)
      ...
      <div class="bg-white rounded-xl shadow-lg p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-700 mb-2">Select Your District</h3>
        <p class="text-gray-500 text-sm">Please select your district from the filter above to view the waste collection schedule.</p>
      </div>
    ...
    @elseif(!empty($message))
      ...
      <div class="bg-white rounded-xl shadow-lg p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-700 mb-2">Notice</h3>
    ...
    @else
            ...
            <tbody class="bg-white divide-y divide-gray-100">
              @foreach($schedules as $index => $row)
                @php
                  ...
                  $isFuture = $d->isFuture();
                @endphp
                <tr class="hover:bg-gradient-to-r hover:from-emerald-50 hover:to-green-50 transition-all duration-200 {{ $isToday ? 'bg-gradient-to-r from-emerald-50 to-green-50 border-l-4 border-emerald-500' : '' }}">
                  <td class="px-6 py-4">
                    <div class="flex items-center space-x-3">
                      ...
                      @endif
                      <div>
                        <div class="text-sm font-bold text-gray-900">{{ $d->format('M d, Y') }}</div>
                        @if($isToday)
                          <span class="text-xs bg-emerald-100 text-emerald-800 px-2 py-1 rounded-full font-medium">Today</span>
                        @elseif($isPast)
                          <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">Past</span>
                        @endif
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                    <span class="text-sm font-medium text-gray-700">{{ $d->format('l') }}</span>
                  </td>
                  <td class="px-6 py-4">
                  ...
                  <td class="px-6 py-4">
                    @php
                      ...
                      $typeColors = [
                        ...
                        'e-waste' => ['bg' => 'bg-purple-100', 'text' => 'text-purple-800'],
                        'metal' => ['bg' => 'bg-gray-100', 'text' => 'text-gray-800'],
                        'glass' => ['bg' => 'bg-cyan-100', 'text' => 'text-cyan-800'],
                        'hazardous' => ['bg' => 'bg-red-100', 'text' => 'text-red-800'],
                        'recyclable' => ['bg' => 'bg-indigo-100', 'text' => 'text-indigo-800'],
                      ];
                      $colors = $typeColors[$wasteType] ?? ['bg' => 'bg-gray-100', 'text' => 'text-gray-700'];
                    @endphp
                  ...
                  <td class="px-6 py-4">
                    <span class="text-sm text-gray-600">{{ $row['notes'] ?? 'Regular collection' }}</span>
                  </td>
                </tr>
              @endforeach
            </tbody>
      ...
      <div class="block md:hidden space-y-4">
        @foreach($schedules as $index => $row)
          ...
          <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden {{ $isToday ? 'ring-2 ring-emerald-400' : '' }}">
            <!-- Card Header -->
            <div class="p-4 {{ $isToday ? 'bg-gradient-to-r from-emerald-50 to-green-50' : 'bg-gradient-to-r from-gray-50 to-emerald-50' }} border-b border-gray-100">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                  @if($isToday)
                  ...
                  @endif
                  <div>
                    <h4 class="text-base font-bold text-gray-800">{{ $d->format('M d, Y') }}</h4>
                    <p class="text-sm text-gray-600">{{ $d->format('l') }}</p>
                  </div>
                </div>
                @if($isToday)
                  <span class="bg-emerald-100 text-emerald-800 px-2.5 py-1 rounded-full text-xs font-medium">Today</span>
                @elseif($isPast)
                  <span class="bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full text-xs font-medium">Past</span>
                @endif
              </div>
            </div>
            ...
            <div class="p-4 space-y-3">
              ...
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Collection Time</p>
              ...
              <div class="flex items-center space-x-3">
                ...
                <div class="flex-1">
                  <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Waste Type</p>
              ...
              <div class="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-700 mt-1">{{ $row['notes'] }}</p>
                </div>
              </div>
              @endif
            </div>

...
<div x-data="historyPage()" class="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-green-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-gray-600 mt-1 text-sm">Track and manage your waste collection reports</p>
        </div>
        
        <a href="{{ route('resident.reports.export.csv') }}" 
           class="flex items-center px-3 sm:px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg font-medium hover:from-blue-600 hover:to-indigo-700 transition-all duration-200 shadow-lg text-sm">
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6">
        ...
        <div class="lg:col-span-2">
          <div class="relative">
            ...
            <input x-model="filters.q" @input.debounce.400ms="load(1)" type="text"
                   ...
                   class="w-full pl-10 pr-4 py-2.5 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 transition-all duration-200 text-sm">
          </div>
        </div>

        <!-- Status Filter -->
        <select x-model="filters.status" @change="load(1)" 
                class="px-3 py-2.5 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 transition-all duration-200 bg-white text-sm">
      ...
      <div class="mt-4 flex justify-end" x-show="filters.q || filters.status || filters.type">
        <button @click="clearFilters()" 
                class="inline-flex items-center px-3 py-1.5 text-gray-600 hover:text-gray-800 text-sm font-medium transition-colors duration-200 hover:bg-gray-50 rounded-lg border border-gray-200">
    ...
    <div class="block xl:hidden">
      <div class="space-y-4" x-show="rows.length">
        <template x-for="r in rows" :key="r.id">
          <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            <!-- Card Header -->
            <div class="p-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-emerald-50">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <div class="font-semibold text-gray-900 text-sm" x-text="fmt(r.created_at)"></div>
                    <div class="text-xs text-gray-500 font-mono" x-text="r.reference_code"></div>
                  </div>
                </div>
            ...
            <div class="p-4">
              <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <div class="text-sm text-gray-800 font-medium truncate" x-text="r.location"></div>
                  </div>
                  <div>
                    <span class="text-xs font-medium text-gray-500 uppercase tracking-wide">Waste Type</span>
                    <div class="mt-1">
                      <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800" x-text="r.waste_type"></span>
                    </div>
                  </div>
                </div>
                ...
                <!-- Progress Timeline -->
                <div>
                  <span class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2 block">Progress</span>
                  <div class="flex items-center gap-1 text-xs">
                    <span :class="dot(r.status,'pending')">●</span><span class="text-gray-600">Submit</span>
                    <span class="text-gray-300">→</span>
                    <span :class="dot(r.status,'assigned')">●</span><span class="text-gray-600">Assign</span>
                    <span class="text-gray-300">→</span>
                    <span :class="dot(r.status,'collected')">●</span><span class="text-gray-600">Collect</span>
                    <span class="text-gray-300">→</span>
                    <span :class="dot(r.status,'closed')">●</span><span class="text-gray-600">Close</span>
                  </div>
                </div>
              ...
              <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
                <a :href="`{{ url('/resident/reports') }}/${r.id}`" 
                   class="inline-flex items-center px-3 py-1.5 text-indigo-600 hover:text-indigo-800 text-sm font-medium transition-colors duration-200 hover:bg-indigo-50 rounded-lg border border-indigo-200">
                  <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button class="inline-flex items-center px-3 py-1.5 text-gray-600 hover:text-gray-800 text-sm font-medium transition-colors duration-200 hover:bg-gray-50 rounded-lg border border-gray-200" @click="duplicate(r)">
                  <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button class="inline-flex items-center px-3 py-1.5 text-red-600 hover:text-red-700 text-sm font-medium transition-colors duration-200 hover:bg-red-50 rounded-lg border border-red-200" 
      ...
      <!-- Mobile Empty State -->
      <div class="bg-white rounded-xl shadow-lg p-8 text-center" x-show="!rows.length && !loading">
        ...
        <div>
          ...
          <p class="text-gray-500 text-sm">Try adjusting your filters or create a new report</p>
        </div>
      </div>
    </div>
    ...
    <div class="hidden xl:block bg-white rounded-xl shadow-lg overflow-hidden">
      <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-100" x-show="rows.length && !loading">
          <template x-for="r in rows" :key="r.id">
            <tr class="hover:bg-gradient-to-r hover:from-emerald-50 hover:to-green-50 transition-all duration-200">
                ...
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div class="mt-1 flex items-center gap-1 text-xs">
                      <span :class="dot(r.status,'pending')">●</span><span class="text-gray-500">Submit</span>
                      <span class="text-gray-300">→</span>
                      <span :class="dot(r.status,'assigned')">●</span><span class="text-gray-500">Assign</span>
                      <span class="text-gray-300">→</span>
                      <span :class="dot(r.status,'collected')">●</span><span class="text-gray-500">Collect</span>
                      <span class="text-gray-300">→</span>
                      <span :class="dot(r.status,'closed')">●</span><span class="text-gray-500">Close</span>
                    </div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-4 py-4">
                <div class="text-sm text-gray-800 font-medium truncate max-w-32" x-text="r.location" :title="r.location"></div>
                <template x-if="r.verified_lat && r.verified_lng">
              ...
              <td class="px-4 py-4">
                <div class="flex flex-wrap gap-1 justify-end">
                  <a :href="`{{ url('/resident/reports') }}/${r.id}`" 
                     class="inline-flex items-center px-2 py-1 text-indigo-600 hover:text-indigo-800 text-xs font-medium transition-colors duration-200 hover:bg-indigo-50 rounded border border-indigo-200">
                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
                    View
                  ...
                  <button class="inline-flex items-center px-2 py-1 text-gray-600 hover:text-gray-800 text-xs font-medium transition-colors duration-200 hover:bg-gray-50 rounded border border-gray-200" @click="duplicate(r)">
                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                    </svg>
                    Copy
                  ...
                  <button class="inline-flex items-center px-2 py-1 text-red-600 hover:text-red-700 text-xs font-medium transition-colors duration-200 hover:bg-red-50 rounded border border-red-200" 
                          @click="cancel(r)"
                          x-show="r.status.toLowerCase()==='pending'">
                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
    ...
    <div class="mt-6 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0" x-show="rows.length || pagination.total">
      <div class="text-sm text-gray-600 bg-white px-4 py-2 rounded-lg shadow-sm" x-text="summary()"></div>
      <div class="flex items-center space-x-2">
        <button class="px-3 py-2 bg-white border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm" 
        ...
        <span class="px-3 py-2 bg-emerald-100 text-emerald-800 rounded-lg text-sm font-medium" x-text="pagination.current_page || 1"></span>
  ...
  <div x-show="map.open" x-cloak x-transition.opacity class="fixed inset-0 z-50">
    ...
    <div class="absolute inset-4 sm:inset-8 lg:inset-auto lg:top-1/2 lg:left-1/2 lg:transform lg:-translate-x-1/2 lg:-translate-y-1/2 bg-white rounded-xl shadow-2xl lg:w-[90%] lg:max-w-2xl lg:h-[500px] p-4">
      <div class="flex justify-between items-center mb-4">
        <h3 class="font-semibold text-base" x-text="'Location for ' + (map.item?.reference_code || '')"></h3>
        <button class="text-gray-700 hover:text-black p-2 hover:bg-gray-100 rounded transition-colors" @click="closeMap()">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
          </svg>
        </button>
  ...
  <div x-show="loading" x-cloak class="fixed inset-0 bg-black/20 flex items-center justify-center z-40">
    <div class="bg-white rounded-xl p-6 shadow-xl">
      <div class="flex items-center space-x-3">
        ...
        <span class="text-gray-700 font-medium">Loading reports...</span>
      </div>
    </div>
  </div>
</div>
  ...
  Alpine.data('historyPage', () => ({
    ...
    chip(s) {
      ...
      return {
        ...
        enroute: 'bg-indigo-100 text-indigo-800 border border-indigo-200',
        collected: 'bg-emerald-100 text-emerald-800 border border-emerald-200',
